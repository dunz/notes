...
> $> `git rebase issue master` or (`issue`를 `master`의 base로 설정하여 `master`에 머지한다.) or<br>
...
$> git cherry-pick ebe6942^..905e279 // range cherry 해당 커밋부터 해당 커밋까지 체리픽한다(오래된..최신)
```
...
상황: 리모트 브랜치명을 `master`에서 `main` 으로 변경
